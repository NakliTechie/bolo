...
- **Look presets** — five backgrounds: **None / Studio / Spotlight / Color / Custom**. Studio and Spotlight are Cap-style gradient bundles with padding, rounded corners, and a soft drop shadow. Color opens a picker for any solid hex; Custom takes any uploaded image (cover-fit). Composited live in the recording canvas, so the styled output goes straight into the WebM.
