...
- **Look presets** — three Cap-style preset bundles (**None / Studio / Spotlight**) that frame the recording with a gradient background, padding, rounded corners, and a soft drop shadow. Composited live in the recording canvas, so the styled output goes straight into the WebM. No fiddly sliders — only intent.
...
- **Post-record annotations** — three Cap-style draw tools (arrow, box, text) drawable on the playback. Each is visible for 3 seconds from the moment you draw it. Render as an SVG overlay live; bake into the export when you choose "Export with overlays burned in".
- **Burn-in export** — re-encode the recording with captions and/or annotations baked into the pixels (so the result works on YouTube / Twitter / embedded players where a sidecar `.srt` doesn't). Drives a canvas + MediaRecorder pipeline from the source video's `requestVideoFrameCallback`, with audio passing through via `srcVid.captureStream().getAudioTracks()`. ~1× real-time — a 5-minute recording takes ~5 minutes to bake.
